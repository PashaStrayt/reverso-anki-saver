...
export function parseCard(cardElement: Element): ParseResult {
  try {
    ...
    if (!word) {
      console.error(
        "[Reverso->Anki] Failed to parse: word not found in page header"
      );
      return { ok: false, reason: "missing_word" };
    }

    // Find the definition text (the main meaning)
    const definitionEl = cardElement.querySelector(
      ".definition-example__mention-sentence"
    );
    if (!definitionEl?.textContent?.trim()) {
      console.error(
        "[Reverso->Anki] Failed to parse: definition not found",
        cardElement
      );
    ...
    // Find examples
    const exampleElements = cardElement.querySelectorAll(
      ".definition-example__example-text-block"
    );
    ...
    const translationElements = cardElement.querySelectorAll(
      "app-translation-chip"
    );
    ...
    if (!translation) {
      console.warn(
        "[Reverso->Anki] No Russian translation found for this card",
        cardElement
      );
    }
